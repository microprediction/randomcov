...
under eleven of the fifteen ensembles. \citet{cheverud2007} concede as much
in passing: in practice the null of no shared structure ``is nearly always
rejected.'' Note however that under \texttt{sparse\_precision}
...
error in the associated EOF is comparable to the neighboring EOF. They draw
the converse as a first-order corollary --- a stand-out eigenvalue has small
EOF sampling error --- while saying plainly that the rule offers no guidance
on where to truncate. It is the corollary we score. Where does this work? We score the
