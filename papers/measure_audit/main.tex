...
random test matrices. That average is of course taken with respect to
some generating law, and in practice the law is usually inherited from
convenience, and from the field. Finance validates on equity panels;
...
locality, and different factor content. So it should be no surprise that
a verdict certified under one law carries no warrant under another. What
...
The correlation-matrix version also has precedent, which is reassuring.
...
structure.\footnote{\texttt{gmarti.gitlab.io/qfin/2019/12/04/hierarchical-risk-parity-part-3.html}}
...
seeded measures, and we tried to keep the naming honest rather than
cute: \texttt{lkj}, \texttt{onion}, \texttt{vine} (elliptope samplers;
...
rewards positions a long-only tree simply cannot take. Against unconstrained
...
ensembles admit near-perfect hedges that only signed weights can reach.
...
identical $(n,T)$ -- more than a fourfold spread for what is supposed to
be an objective count. The same sweep scores the eigenvalue-greater-than-one rule of \citet{kaiser1960} against the population count of
...
under \texttt{sparse\_precision}. So the number of factors a study reports
is, in part, a property of the generating measure its data happen to resemble.
...
ratio of true output power, loaded over unloaded. The picture is genuinely mixed: loading halves the
output power under half the ensembles, which is the win it is sold as. But it multiplies the power under
\texttt{archakov\_hansen} ($21$), \texttt{animals} ($33$),
\texttt{kernel} ($136$), and \texttt{walk} ($10^{7.9}$), which is not a small miss. The mechanism
...
\texttt{animals} are nearly honest ($0.90$, $0.89$) simply because their truth
is already low-dimensional. Across all fifteen ensembles the estimate
never exceeds $5.2$, whatever the truth -- which should give anyone pause. A low reported $n_D$ cannot by
itself distinguish genuinely low-dimensional biology from a noisy estimate of
diffuse structure.
...
dominates at $n=30$ and would wash out with more assets. So we checked.
We re-run the Ledoit--Wolf intensity demonstration of Section~\ref{sec:lw-demo} and five of the seven
...
where compute starts to bite). Figure~\ref{fig:scale} plots all thirty-five cells of every audit, one
line per ensemble; the two largest passes alone cost on the order of a
hundred worker-hours apiece, so this was not a check we could afford to run casually.

The short answer is that the spread does not close. At $n=3000$, $T/n=1$, the Ledoit--Wolf
intensity still ranges from $0.002$ (\texttt{factor}) to $0.780$
(\texttt{sparse\_precision}), if anything a wider ratio than the $0.157$--$0.720$
...
turns out to be a reliable predictor of its verdict at a hundred times
the dimension.
...
moves to $1.259$ and $2.580$, both apparent losses. That surprised us
enough that we ran it back. A targeted
...
estimate had simply landed on the high side of a noisy statistic -- a
good reminder that a rep count that is fine for most cells is not
automatically fine for all of them.
...
tested. Only \texttt{hierarchical} genuinely flips, presumably because a fixed
...
program keeps exact use of the full covariance regardless of $n$. So a
claim's home-turf verdict is not guaranteed to survive scale. And, as we
just saw, a low rep count is not guaranteed to survive a check either.

So we went looking for more of the same. At $n=3000$, $T/n=2$, three
...
paper, same seeding scheme as the HRP check.} Nothing moves. The
difference from HRP is the statistic itself: a Frobenius-norm ratio has
almost no per-draw variance, while HRP's long-only optimizer clearly
does, so twelve draws are trustworthy for one and not for the other. The
fragility we found is a property of the audit, not of small rep counts
in general.
...
OAS and fixed shrinkage and the graphical lasso on three each, and,
perhaps surprisingly, the raw sample covariance on \texttt{animals} and \texttt{kernel}, the two
...
\texttt{residuals}, and \texttt{factor} -- a big fall. The \texttt{precise}
estimators are built for time-varying covariance and pay for that design
on static panels, filling the lower ranks, which seems fair enough. The upshot is that
a library default is really a bet on a measure, whether or not the author meant it that way.
...
actually teaches a taxonomy of its own. Claims that impose structure on the truth are
ensemble-relative; theorems about Wishart sampling noise, on the other hand, hold for
every true covariance and simply cannot flip.

The structural claims are naturally the audit targets. \citet{paz2015} taper clustering-statistic covariances and presume banded truth;
...
the predictor eigenstructure. Each of these, in our view, carries a predicted flip waiting to be checked.

The invariant theorems serve as null controls, and we lean on them deliberately. \citet{hartlap2007}
...
a known covariance only through a noncentrality parameter. Put simply, an audit
...
everywhere. Good -- the pipeline reports no ensemble-dependence where none can
exist. If anything, the most instructive failures are invariant theorems wrapped in
