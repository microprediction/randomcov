...
ensembles reach the positive-semidefinite floor, so the exponent is partly
a statement about the floor: it scales inversely with the clip, and a
looser clip moves it down decade for decade. The direction is a property
of the ensemble; the magnitude is not.
...
\texttt{hierarchical} ($0.76$). The mechanism is not quite the textbook
one. \texttt{hierarchical} does carry a spike over a bulk, but
\texttt{sparse\_precision} carries none --- its largest true eigenvalue is
$1.43$ against an edge of $2.91$ --- and clipping wins there because the
sub-edge spectrum is nearly flat already, so flattening it costs almost
nothing. What predicts the verdict is the dispersion of the true spectrum
below the edge, not the presence of spikes above it. On the other hand
clipping damages the estimate under eleven of fifteen ensembles, worst at
$1.43$--$1.77$ under \texttt{animals}, \texttt{kernel},
\texttt{archakov\_hansen}, and \texttt{residuals}, where the sub-edge mass
is dispersed and is erased as noise. The recipe presumes signal
...
$1.00$), \texttt{hierarchical}, and \texttt{block\_equicorr}. It also reads
...
there is nothing for five neighbors to miss. It fails several-fold under the elliptope samplers \texttt{lkj},
...
field on the identical point clouds gives $1.04$ instead --- which is
Stein's caveat, reproduced as a contrast rather than assumed.
...
elliptope middle. But it loses under \texttt{factor} ($1.09$), and under
\texttt{residuals} it ties at $1.00$ while winning on none of the twenty
draws. No mystery here. Dense low-rank structure is the worst case for
sparsity.
...
$100\%$ of the time under eleven of the fifteen ensembles. \citet{cheverud2007} concede as much
...
tested. Only \texttt{hierarchical} genuinely flips, and the cause is the
...
hundredfold. HRP's home turf evaporates under it, while the long-only quadratic
