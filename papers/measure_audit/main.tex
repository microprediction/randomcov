...
% Run-in headings italic rather than article's bold (house style).
\makeatletter
\renewcommand\paragraph{\@startsection{paragraph}{4}{\z@}%
  {3.25ex \@plus1ex \@minus.2ex}{-1em}%
  {\normalfont\normalsize\itshape}}
...
stated. We measure how much that omission matters by re-running eleven
published claims from eleven fields under fifteen named, seeded
ensembles at identical dimension and sample size. Every claim flips
verdict as the ensemble changes, for stated structural reasons, and a
joint leaderboard of scikit-learn and \texttt{precise} estimators has a
different winner under different measures. The pattern survives scale:
the verdicts at $n=30$ predict those at $n=3000$ for every claim but
hierarchical risk parity, which flips under its own home-turf ensemble
as dimension grows. The \texttt{randomcov} library names the measures
and seeds the draws, so any claim can be re-run under all fifteen, and a
docket of further claims from astronomy, genomics, neuroscience, and
chemometrics, with ensemble-invariant null controls, is assembled for
the next round.
\end{abstract}
...
The present paper adds the instrument and the audits: a fixed
...
\section{Verdicts from $n=30$ to $n=3000$}\label{sec:scale}
