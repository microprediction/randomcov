...
under \texttt{kernel} fields it is $4\times10^{-3}$, because the fit does
not yet admit the band-diagonal structure a kernel covariance calls for.

A validation run on any single ensemble would have reported either
...
library, and report the verdict. The selection leaned toward claims we
knew to be useful or hoped would be, so a flip here is evidence about a
claim's scope, not about its worth.
...
loosens. The ensemble sets the direction of the result. The floor sets its
size.
...
ensemble that genuinely is local,
...
The sparsity prior wins clearly where precisions are genuinely sparse or
banded: $0.63$ under \texttt{sparse\_precision}, $0.68$ under
\texttt{hierarchical}, $0.69$ under \texttt{ar1}. It ties across the dense
elliptope middle. It loses
...
panels, filling the lower ranks. The upshot is that
...
yields a taxonomy. Claims that impose structure on the truth are
...
The structural claims are naturally the audit targets. \citet{paz2015} taper clustering-statistic covariances, declining to
