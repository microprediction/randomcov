...
follows --- flatten every eigenvalue below the edge $(1+\sqrt{n/T})^{2}$
and report a cleaner matrix --- is \citet{laloux2000}. Column~2 gives the
...
compared against the mean null eigenvalue, and we use the now-standard
95th-percentile form of \citet{glorfeld1995}; it is the standard
...
\citep{houtekamer2001,hamill2001}. \citet{paz2015} apply the closely related covariance tapering of
\citet{kaufman2008} to correlation-function bins, with a Wendland taper and a
two-taper precision estimator; the assumption both share --- that the taper
coordinate carries the truth's locality --- is what this audit tests, so it
applies to both fields. With $N=20$ members for $n=30$ variables and a
...
precision matrix cheap enough to be routine --- their own claim is speed, not
accuracy --- and the field attached to it the promise that the prior improves
estimation. That promise underwrites partial-correlation connectomes
...
networks. And indeed we see this. The sparsity prior wins clearly where precisions are
...
\paragraph{Climatology.} \citet{north1982} warn that a leading EOF is untrustworthy when its
eigenvalue sampling error $\lambda\sqrt{2/T}$ is comparable to the gap to the
next eigenvalue. In practice the rule is read in the other direction, as a
licence to trust a well-separated leading EOF, and it is that reading we
score. Where does this work? We score the
...
The structural claims are naturally the audit targets. \citet{paz2015} taper clustering-statistic covariances --- explicitly
declining to assume the far off-diagonal entries are negligible, though the
taper still presumes the bin coordinate carries the locality;
...
independent data of the same shape, which is to say against an ensemble. \citet{ledoit2012} show the edge of nonlinear over linear shrinkage growing
...
eigenstructure. Each of these, in our view, carries a predicted flip waiting to be checked.
